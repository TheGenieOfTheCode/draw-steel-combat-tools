...
export class DamageConditionsPanel extends ds.applications.api.DSApplication {
  ...
  async _prepareContext(_options) {
    return {
      ...
      conditions:      [{ value: '', label: '-- None --', selected: !this._condition }, ...ALL_CONDITIONS.filter(c => !c.requiresSource).map(c => ({ value: c.id, label: c.label, selected: c.id === this._condition }))],
      sourceConditions: ALL_CONDITIONS.filter(c => c.requiresSource).map(c => ({ value: c.id, label: c.label, selected: c.id === this._condition })),
